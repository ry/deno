...
testPerm({ net: true }, async function fetchMultipartFormDataSuccess():Promise<void> {
...
testPerm({ net: true }, async function fetchURLEncodedFormDataSuccess():Promise<void> {
  const response = await fetch(
...
testPerm({ net: true }, async function fetchInitTypedArrayBody():Promise<void> {
  const data = "Hello World";
  const response = await fetch("http://localhost:4545/echo_server", {
    method: "POST",
...
testPerm({ net: true }, async function fetchInitURLSearchParamsBody():Promise<void> {
