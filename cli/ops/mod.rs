...
pub fn dispatch_all(
  ...
  let op = if op_id != FLATBUFFER_OP_ID {
    let min_record = dispatch_minimal::parse_min_record(control).unwrap();
    dispatch_minimal::dispatch(state, op_id, min_record, zero_copy)
  } else {
...
pub fn op_selector_std(inner_type: msg::Any) -> Option<CliDispatchFn> {
  match inner_type {
    ...
    // Read, Write op are defined in dispatch_minimal
    msg::Any::Read => unreachable!(),
    msg::Any::Write => unreachable!(),
